...
/*
 * Version 10: Torque Controller - With Torque control setting turned ON
 * Author: Loui Byrne
 * Date: 30 August 2023
 * Details: Masters Thesis Project - Developing an ADCS CubeSat Testbed
 * Location: University College Dublin, Ireland
 * USER SETUP STEPS:
 * a) "Project Properties > C/C++ Build > Settings > MCU Settings > Check Box: "Use float with printf from newlib-nano"
 * b) Change to release mode
 * c) Enable .oih build file type
 * d) Add Siumlink folder to path
 */
...
#include <string.h>
#include <Torque_Controller.h> 	// Include Simulink Algorithm Header
...
int main(void)
{
	...
	// SET MANOUVRE PARAMETERS HERE!!!
	//float Target_Heading_0 = 2; // Radians
	int Update_Interval_1 = 15; // Seconds
	float Target_Heading_1 = 3; // Radians
	int Update_Interval_2 = 15; // Seconds
	float Target_Heading_2 = 6; // Radians
	float Target_Heading = 0 ; // Radians

	// Slew Manouver Settings
	int Loop_Counter = 0;						// Keep track of loop time to assign target heading
	int Seconds_to_Loops = 15;					// 1 second = 150 loops [**CALIBRATE**]
	//45 seconds = 500 s/l in torque mode w/ no serial debug
	...
	void Arduino_SerialTx() {
		...
		int RW_Speed_S32 = (int)Motor_Speed_RPM; // No decimal points
		//int RW_Speed_S32 = (MC_GetMecSpeedAverageMotor1()*6); // Speed units are 1/6 rpm
		...
		// Target Euler Heading
		//int Target_Heading_S32 = (int)rtY.Ang_error; // No decimal points
		int Target_Heading_S32 = Target_Heading; // No decimal points
		uint8_t TH[4]; // Element 0 is LSB of LC1_S32, Element 3 is MSB
		TH[0] = Target_Heading_S32 & 0xFF; // LSB (0xFF = 0b11111111)
		TH[1] = (Target_Heading_S32 >> 8) & 0xFF;
		TH[2] = (Target_Heading_S32 >> 16) & 0xFF;
		TH[3] = (Target_Heading_S32 >> 24) & 0xFF; // MSB
		...
		uint8_t TxBuffer_Serial_Debug[20] = { IMU[0], IMU[1], IMU[2], IMU[3], LC1[0], LC1[1],
				...
				RW_Speed[2], RW_Speed[3] , TH[0], TH[1], TH[2], TH[3]}; // 16 bytes holding 4 uint32_t (Little Edian LSB stored First)
...
//		uint8_t RxBuffer = 0;
//		HAL_UART_Receive(&huart3, &RxBuffer, 1, 10);
//		printf(" R//printfr: %d ", RxBuffer);
//		if (RxBuffer != 0){ // For now we are only interested in uplinking the target euler heading 8 bit is fine (1-8 > 1-6 radians)
//			Target_Euler_Heading = RxBuffer * 1.0; // Convert to Float
//		}
//	}
	...
	void Read_Euler_Heading() {
		...
		//printf(" IMU: %.3f ", Euler_Heading_Rad);
	}
	...
	void BNO055_Init() {
		...
		if (Mag_Calibration_Status == 3) {
			//printf("\//printfESS - IMU Calibrated\n");
	...
	void Read_Load_Cells(int samples) {
		...
		//printf("\n LC1: %0.4f LC2: %0.4f ", LoadCellRead[0], LoadCellRead[1]); //Only The first 4 decimal places are useful!
	}
...
//		Ramp_Time = 10; // [Tunable Parameter]
//
//		// Motor Control
...
//	}
	...
	void Simulink_Motor_Control(float Motor_Speed_RPM) {
		...
		MC_StartMotor1(); // Restart motor if it has faulted
		//MC_ProgramSpeedRampMotor1_F(Motor_Speed_RPM, Ramp_Time); //Speed is rpm
		//Motor_Speed_RPM = Motor_Speed_RPM/100;
		Ramp_Time = 10;
