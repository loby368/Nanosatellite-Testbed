...
int main(void) {
	...
	// Nano Variables
	int LoadCellRead_Initial[2] = { 0, 0 };		// Initial Load Cell readings in Newtons
	...
	/*
	 // Proportional motor control using IMU only
	 void Simple_Motor_Control() {

	 // Define Performace Parameters
	 float Motor_Speed_Limit = 3000;
	 float Motor_Dead_Zone = 250;
	 ...
	 MC_AcknowledgeFaultMotor1(); //Aknowledge Fault
	 MC_StartMotor1(); //Restart motor if it has faulted
	 MC_ProgramSpeedRampMotor1_F(Motor_Speed_RPM, Ramp_Time); //Speed is in 1/6 rpm
	 HAL_Delay(Ramp_Time);
	...
	void Simulink_Motor_Control(float Motor_Speed_RPM) {
		...
		// Fault Aknowledgement procedure
		//		uint16_t Current_Fault = MC_GetCurrentFaultsMotor1();
		//		if (Current_Fault != 0){ // Need to look up error codes in datasheet!!!
		//			MC_AcknowledgeFaultMotor1(); //Aknowledge Fault
		//			MC_StartMotor1(); //Restart motor if it has faulted
		//		}
	...
	void Arduino_Debug() {
		...
		// IMU Data
		int IMU_S32 = (int)((Euler_Heading_Rad - Initial_Euler_Heading_Rad) * 1000); //Scale by 1000 to preserve 3 decimal places
		printf("\n\nIMU_S32 = %i\n", IMU_S32);
		// Write int32_t into 4x Bytes
		...
		printf("\n\nIMU_Bytes = %d %d %d %d\n", IMU[3], IMU[2], IMU[1], IMU[0]);

		// Load Cell 1 Data
		int LC1_S32 = (int)((LoadCellRead[0] - LoadCellRead_Initial[0]) * 10000); //Scale by 10,000 to preserve 4 decimal places
		printf("\n\nLC1_S32 = %d\n", LC1_S32);
		// Write int32_t into 4x Bytes
		...
		printf("\n\nLC1 = %d %d %d %d\n", LC1[3], LC1[2], LC1[1], LC1[0]);

		// Load Cell 2 Data
		int LC2_S32 = (int)((LoadCellRead[1] - LoadCellRead_Initial[1]) * 10000); //Scale by 10,000 to preserve 4 decimal places
		printf("\n\nLC2_S32 = %d\n", LC2_S32);
		// Write int32_t into 4x Bytes
		...
		printf("\n\nRW_Speed = %d %d %d %d\n", RW_Speed[3], RW_Speed[2], RW_Speed[1], RW_Speed[0]);
		...
		uint8_t TxBuffer_Serial_Debug[16] = { IMU[0], IMU[1], IMU[2], IMU[3], LC1[0], LC1[1],
				...
				RW_Speed[2], RW_Speed[3] }; // 16 bytes holding 4 uint32_t
	...
	while (1) {
		...
		/* Read Load cells --------------------------------------------------------*/
		Read_Load_Cells(2);
		rtU.Load_Cell_1_Input = LoadCellRead[0]
				- LoadCellRead_Initial[0] / calibrationFactor;
		rtU.Load_Cell_2_Input = LoadCellRead[1]
				- LoadCellRead_Initial[1] / calibrationFactor;
