...
int main(void)
{
  /* USER CODE BEGIN 1 */
	...
	float STM32_Regiter_Data_Scaled;
	...
	int Ramp_Time = 1; 						// [Tunable parameter] - 10ms should be fine?
...
//		float x = 1.2345;
//		memcpy(&TxBuffer_Serial_Debug[0], &x, sizeof(Euler_Heading_Rad));
//		memcpy(&TxBuffer_Serial_Debug[4], &x, sizeof(LoadCellRead[0]));
//		memcpy(&TxBuffer_Serial_Debug[8], &x, sizeof(LoadCellRead[1]));
//		memcpy(&TxBuffer_Serial_Debug[12], &x, sizeof(Motor_Speed_RPM));
		...
		HAL_UART_Transmit(&huart3, TxBuffer_Serial_Debug, 16, 10);

//		printf("\n");
//		for (int i =0; i<16; i+=4){
//			printf("\nTxBuffer: %d, %d, %d, %d \n", TxBuffer_Serial_Debug[i+3], TxBuffer_Serial_Debug[i+2], TxBuffer_Serial_Debug[i+1], TxBuffer_Serial_Debug[i]);
//		}
	}
	...
	void Arduino_SerialRx() {
		...
		HAL_UART_Receive(&huart3, RxBuffer, 5, 10);
		// Decode RxBuffer
		...
		STM32_Regiter_Data_Scaled = STM32_Regiter_Data / 10000.0; // Scaling factor to allow 5 dvcimal places to be sent
		printf(" Addr: %d Data: %.5f\n", STM32_Register_Adddress, STM32_Regiter_Data_Scaled);

//		// Set STM32 Registers with uplkinked Data
//		if (STM32_Regiter_Data_Scaled != 0)
//
//	}
